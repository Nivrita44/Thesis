...
test('obstacle exactly between in same col', () => {
    const input = [{ x: [1, 1], y: [1, 4], f: [1, 3] }];
    expect(solve(input)).toEqual([4]);
});
...
// ●
// obstacle exactly between in same col
...
// 32 | test('obstacle exactly between in same col', () => {
//     33 |
//         const input = [{ x: [1, 1], y: [1, 4], f: [1, 3] }]; >
//     34 | expect(solve(input)).toEqual([4]); |
//     ^
//     35 |
// });
// 36 |
//     37 | test('no obstacle, unrelated f', () => {

//             at Object.toEqual(tests - algorithm / tests - prompt2 / 28 _prompt2.test.js: 34: 24)
