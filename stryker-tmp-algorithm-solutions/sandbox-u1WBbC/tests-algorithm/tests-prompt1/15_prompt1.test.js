...
test('sample 1: e2 to e4', () => {
    ...
    expect([
        ['U', 'U'],
        ['U', 'U']
    ]).toContainEqual(result.directions);
});

test('sample 2: a1 to h8', () => {
    const result = solve('a1', 'h8');
    expect(result.count).toBe(7);
    expect(result.directions.length).toBe(7);
    const diagMove = result.directions.every(move => move === 'RU');
...
test('sample 3: d4 to d4', () => {
    ...
    expect(result.directions.length).toBe(0);
});
...
test('sample 4: h8 to a1', () => {
    ...
    expect(result.count).toBe(7);
    expect(result.directions.length).toBe(7);
    const diagMove = result.directions.every(move => move === 'LD');
    expect(diagMove).toBe(true);
});
...
test('sample 5: a8 to h1', () => {
    ...
    expect(result.count).toBe(7);
    expect(result.directions.length).toBe(7);
    const diagMove = result.directions.every(move => move === 'RD');
    expect(diagMove).toBe(true);
});
...
test('sample 6: a1 to a2', () => {
    ...
    expect(['U', 'D', 'L', 'R', 'LU', 'LD', 'RU', 'RD']).toContain(result.directions[0]);
});
...
test('sample 7: a1 to b2', () => {
    ...
    expect(result.count).toBe(1);
    expect(result.directions.length).toBe(1);
    expect(['RU']).toContain(result.directions[0]);
});

test('sample 8: c3 to f4', () => {
    const result = solve('c3', 'f4');
    expect(result.count).toBe(3);
    expect(result.directions.length).toBe(3);
});
...
test('sample 10: h1 to h8', () => {
    ...
    expect(result.count).toBe(7);
    expect(result.directions.length).toBe(7);
    const isU = result.directions.every(m => m === 'U');
