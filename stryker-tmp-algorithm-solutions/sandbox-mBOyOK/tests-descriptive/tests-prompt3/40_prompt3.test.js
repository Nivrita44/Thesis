...
test('Simple n=2, alternating removed at iteration 1', () => {
    expect(solve(2, [1, -1])).toEqual([1, 2]);
    expect(solve(2, [-1, 1])).toEqual([2, 1]);
});

test('n=3, only middle remains, others removed', () => {
    const out = solve(3, [1, -1, 1]);
...
// FAIL tests - descriptive / tests - prompt3 / 40 _prompt3.test.js× Simple n = 2, alternating removed at iteration 1(12 ms)× n = 3, only middle remains, others removed(3 ms)√ n = 4, various patterns(2 ms)√ All elements except one removed in first iteration√ n = 6, more iterations(1 ms)√ Middle element survives longest(1 ms)√ n = 8, elements removed in different iterations(2 ms)√ Edge
// case :only one - 1(n = 2)(1 ms)√ Edge
// case :large n odd(4 ms)√ Edge
// case :large n even(1 ms)√ All at first iteration except last(1 ms)√ Distinct iterations(1 ms)√ n = 5, varying pattern(1 ms)√ Last element is survivor(1 ms)√ First element is survivor(3 ms)√ Multiple - 1 s not possible(problem constraint), single survivor(4 ms)√ Stress:
//     all elements removed at step 1 except survivor(1 ms)

//     ● Simple n = 2, alternating removed at iteration 1
...
//     2 |
//         3 | test('Simple n=2, alternating removed at iteration 1', () => { >
//             4 | expect(solve(2, [1, -1])).toEqual([1, 2]); |
//             ^
//             5 | expect(solve(2, [-1, 1])).toEqual([2, 1]);
//             6 |
//         });
//     7 |

//         at Object.toEqual(tests - descriptive / tests - prompt3 / 40 _prompt3.test.js: 4: 29)

//     ● n = 3, only middle remains, others removed
...
//     9 |
//         const out = solve(3, [1, -1, 1]);
//     10 | expect(new Set(out)).toEqual(new Set([1, 2, 3])); >
//     11 | expect(out[1]).toBe(2); |
//     ^
//     12 |
// });
//     13 |
//         14 | test('n=4, various patterns', () => {

//                 at Object.toBe(tests - descriptive / tests - prompt3 / 40 _prompt3.test.js: 11: 18)
