...
const websiteConfigs = {
  ...
  demoqa1: {
   configFile: "./config/demoqa.js",
   testFile1: "demoqa_chunk1.test.js",   // not actually used, but kept for fallback
   testFile2: "demoqa_chunk2.test.js",   // not actually used, but kept for fallback
   logFile1: "../logs/demoqa_chunk1_results.txt",
   logFile2: "../logs/demoqa_chunk2_results.txt"
 },
  magento: {
...
function generateDemoQAPrompt1_chunk1(config, inputsContext) {
   return `
 Write a **complete runnable Selenium WebDriver test in Node.js** that performs full end-to-end flow on DemoQA website with PROPER TIMING and STABILITY.
 
 // ... [all imports and setup as in your original prompt]
 - Import { Builder, By, until, Key } from 'selenium-webdriver'.
 - Import { Options } from 'selenium-webdriver/chrome.js'.
 ...
 - Define logFilePath using path.join(__dirname, '../logs/demoqa_results.txt')
 - CRITICAL: Create logs directory if it doesn't exist using fs.mkdir(path.dirname(logFilePath), { recursive: true })
 - Implement helper 'logResult(testName, status)' using fs.writeFile to overwrite '../logs/demoqa_chunk1_results.txt'.
 - Each test must log its individual result: 'PASS' or 'FAIL' with timestamp
 - Log format: '[timestamp] Test: [testName] - Status: [PASS/FAIL]'
 - Each test should log its result immediately after completion
 - At the end, log a summary: 'Test Summary: X passed, Y failed out of 5 total tests'
 ...
 - The config file is in config/ folder (one level up from e2e_tests/)
 - Use '../config/demoqa.js' for the import path (NOT './config/demoqa.js')
 - Use '../logs/demoqa_chunk1_results.txt' for the log file path
 - CRITICAL: Create logs directory if it doesn't exist using fs.mkdir(path.dirname(logFilePath), { recursive: true })
 ...
 function generateDemoQAPrompt1_chunk2(config, inputsContext) {
   return `
 Write a **complete runnable Selenium WebDriver test in Node.js** that performs full end-to-end flow on DemoQA website with PROPER TIMING and STABILITY.
 ...
 - Import { fileURLToPath } from 'url'.
 - CRITICAL: Define __filename and __dirname for ES modules: const __filename = fileURLToPath(import.meta.url); const __dirname = path.dirname(__filename);
 - Define logFilePath using path.join(__dirname, '../logs/demoqa_chunk2_results.txt')
 - CRITICAL: Create logs directory if it doesn't exist using fs.mkdir(path.dirname(logFilePath), { recursive: true })
 - Implement helper 'logResult(testName, status)' using fs.writeFile to overwrite '../logs/demoqa_chunk2_results.txt'.
 - Each test must log its individual result: 'PASS' or 'FAIL' with timestamp
 - Log format: '[timestamp] Test: [testName] - Status: [PASS/FAIL]'
 - Each test should log its result immediately after completion
 - At the end, log a summary: 'Test Summary: X passed, Y failed out of 5 total tests'
 ...
 - The config file is in config/ folder (one level up from e2e_tests/)
 - Use '../config/demoqa.js' for the import path (NOT './config/demoqa.js')
 - Use '../logs/demoqa_chunk2_results.txt' for the log file path
 - CRITICAL: Create logs directory if it doesn't exist using fs.mkdir(path.dirname(logFilePath), { recursive: true })
 ...
 - When performing double click or right click actions:
  - Use "await driver.actions({bridge: true})" for creating actions.
  - Before the action, verify element visibility and enabled state explicitly.
  - After performing action, wait 2 seconds for response.
  - If action fails, catch error, log failure, and optionally retry once.
...
async function generateTestsForWebsite(websiteName) {
 ...
 `;
   ...
   // ==== SPECIAL CHUNK LOGIC FOR demoqa1 ====
   if (websiteName === 'demoqa1') {
     // --- Generate Chunk 1 ---
     const prompt1_chunk1 = generateDemoQAPrompt1_chunk1(websiteConfig, inputsContext);
     const testCode_chunk1 = await generateTestCode(prompt1_chunk1); 
     const testPath_chunk1 = path.join(__dirname, "e2e_tests", "demoqa_chunk1.test.js");
     await saveTestFile(testPath_chunk1, testCode_chunk1);
     console.log(`✅ demoqa1 Test Chunk 1 Finished!`);
     console.log(`Run with: node ${path.relative(__dirname, testPath_chunk1)}`);
 
     // --- Generate Chunk 2 ---
     const prompt1_chunk2 = generateDemoQAPrompt1_chunk2(websiteConfig, inputsContext);
     const testCode_chunk2 = await generateTestCode(prompt1_chunk2);
     const testPath_chunk2 = path.join(__dirname, "e2e_tests", "demoqa_chunk2.test.js");
     await saveTestFile(testPath_chunk2, testCode_chunk2);
     console.log(`✅ demoqa1 Test Chunk 2 Finished!`);
